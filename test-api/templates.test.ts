		...
		it("should return a template by id (200)", async () => {
			...
			await $fetch(`/templates/${templateId}`, {
				...
				onResponse: ({ response }) => {
					...
					expect(Array.isArray(response._data.categories[0].ingredients)).toBe(
						true,
					);
					expect(response._data.categories[0].ingredients.length === 1).toBe(
						true,
					);
				},
			});
		...
		it("should apply template to user (200)", async () => {
			...
			await $fetch(`/templates/${templateId}/apply`, {
				...
				onResponse: ({ response }) => {
					...
					expect(Array.isArray(response._data.data.createdCategories)).toBe(
						true,
					);
