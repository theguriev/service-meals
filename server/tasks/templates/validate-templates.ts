...
async function validateTemplateFile(
	...
	try {
		...
		if (templateName === defaultTemplateName && filename !== "default.json") {
			result.errors.push(
				"Cannot use default template name for non-default template",
			);
		}
		...
		templateData.categories.forEach(
			(category: TemplateCategory, categoryIndex: number) => {
				...
				if (category.ingredients.length === 0) {
					result.warnings.push(
						`Category "${category.name}" has no ingredients`,
					);
				}

				// Валидируем ingredients
				category.ingredients.forEach(
					(ingredient: TemplateIngredient, ingredientIndex: number) => {
						totalIngredients++;
						...
						if (
							...
							(typeof ingredient.grams !== "number" || ingredient.grams <= 0)
						) {
							result.errors.push(
						...
						if (
							ingredient.isAlcohol !== undefined &&
							ingredient.isAlcohol !== null &&
							typeof ingredient.isAlcohol !== "boolean"
						) {
							result.errors.push(
