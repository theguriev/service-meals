...
export default defineEventHandler(async (event) => {
	...
	const { withIngredients } = await zodValidateData(
		getQuery(event),
		querySchema.parse,
	);
	...
	const categoryMatch = can(user, "get-all-categories")
		? { _id: new ObjectId(id) }
		: {
				_id: new ObjectId(id),
				userId,
	...
	if (!can(user, "get-all-ingredients")) {
		ingredientsLookup.$lookup.pipeline = [{ $match: { userId } }];
	}

	const category = (
		await ModelCategories.aggregate([
			{ $match: categoryMatch },
